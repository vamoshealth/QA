...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    #lock-screen{position:fixed;inset:0;background:#f9fafb;display:flex;align-items:center;justify-content:center;z-index:999}
  </style>
</head>
<body>
<div id="lock-screen">
  <div style="background:#fff;border-radius:16px;padding:36px 32px;border:1px solid #e5e7eb;width:100%;max-width:360px;text-align:center">
    ...
    <input id="pw-input" type="password" placeholder="Contraseña..." style="margin-bottom:12px;text-align:center"/>
    <button id="pw-btn" style="width:100%;padding:10px 0;background:#7F77DD;color:#fff;border:none;border-radius:8px;font-size:14px;font-weight:700">Entrar</button>
...
const CATS=[
  ...
  {id:"objections",label:"Objeciones",label_en:"Objections",color:"#D85A30",bg:"#FAECE7",tc:"#712B13",
   es:["Recibió objeciones con calma","Clarificó la objeción antes de responder","Respondió al precio con valor — NO planes de pago","Usó herramientas en orden: valor → urgencia → referido → descuento → exención","Confirmó resolución antes de continuar"],
   en:["Received objections calmly","Clarified objection before responding","Addressed price with value — NOT payment plans","Used approved tools in order","Confirmed objection resolved before moving on"]},
  {id:"closing",label:"Cierre",label_en:"Closing",color:"#639922",bg:"#EAF3DE",tc:"#27500A",
   es:["Pidió la venta directamente","Intentó cerrar el mismo día por teléfono","Usó técnica adecuada: asuntivo, alternativa o urgencia","Ante dudas profundizó con preguntas","Aseguró próximo paso concreto — 'Hablamos pronto' = falla"],
  ...
  {id:"opportunities",label:"Oportunidades Perdidas",label_en:"Missed Opportunities",color:"#888780",bg:"#F1EFE8",tc:"#2C2C2A",
   es:["Señal de compra ignorada","Oportunidad familiar perdida — no se exploró plan, descuentos o chequeos","Dolor mencionado sin profundizar","Bono de $50 por referido no mencionado","Vamos Juntos no usado cuando el prospecto dudaba","Exención de inscripción no intentada cuando la llamada se cayó por el costo"],
   en:["Buying signal ignored","Family opportunity missed — plan/discounts/school physicals not explored","Pain mentioned but not explored","$50 referral bonus not mentioned","Vamos Juntos not used when prospect hesitated","Enrollment fee waiver not attempted when call died over fee"]}
];
...
if(savedPw){verifyAndUnlock(savedPw);}
...
function render(){
  ...
  hLeft.appendChild(hTitles);hdr.appendChild(hLeft);
  ...
  [["analyze","Analizar"],["history","Historial"],["trends","Tendencias"],...(state.viewing?[["view","Reporte"]]:[])]
    .forEach(([id,label])=>{const b=el("button",{className:"tab"+(state.tab===id?" active":""),onClick:()=>{state.tab=id;if(id==="history"||id==="trends")loadHistory();render();}},label);tabBar.appendChild(b);});
...
function renderAnalyze(app){
  ...
  const ri=el("input",{type:"text",placeholder:"Ej. Maria Garcia",value:state.rep,onInput:e=>state.rep=e.target.value});rw.appendChild(ri);
  const dw=el("div");dw.appendChild(el("div",{style:{fontSize:"11px",fontWeight:"600",color:"#374151",marginBottom:"4px"}},"Fecha"));
  const di=el("input",{type:"date",value:state.date,onInput:e=>state.date=e.target.value});dw.appendChild(di);
...
function renderResult(wrap,r){
  ...
  CATS.forEach(c=>{
    ...
    scores.forEach((s,i)=>{
      ...
      const ri2=el("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:"8px"}});
      ri2.appendChild(tw);
      ri2.appendChild(el("span",{className:"badge badge-"+(s==="pass"?"pass":s==="fail"?"fail":"na")},s==="pass"?"✓ Pass":s==="fail"?"✗ Fail":"— N/A"));
      row.appendChild(ri2);
      if(s==="fail"){
...
function renderTrends(app){
  ...
  const byRep={};
  state.hist.forEach(h=>{
    const name=h.repName||"Unknown";
    if(!byRep[name])byRep[name]=[];
    byRep[name].push(h);
  });
  const title=el("div",{style:{fontWeight:"800",fontSize:"15px",marginBottom:"16px"}},"Tendencias por Rep");
  ...
  Object.entries(byRep).forEach(([name,calls])=>{
    ...
    const trend=scores.length>1?latest-scores[0]:0;
    ...
    const avgBadge=el("div",{style:{textAlign:"center",background:vm.bg,border:"1px solid "+vm.border,borderRadius:"8px",padding:"5px 12px"}});
    avgBadge.appendChild(el("div",{style:{fontSize:"16px",fontWeight:"800",color:vm.text}},avg+"%"));
    avgBadge.appendChild(el("div",{style:{fontSize:"9px",color:vm.text}},"Promedio"));
    rr.appendChild(avgBadge);
    if(scores.length>1){
      const trendColor=trend>0?"#085041":trend<0?"#991B1B":"#6b7280";
      const trendBg=trend>0?"#E1F5EE":trend<0?"#FEE2E2":"#f3f4f6";
      const trendEl=el("div",{style:{textAlign:"center",background:trendBg,borderRadius:"8px",padding:"5px 12px"}});
      trendEl.appendChild(el("div",{style:{fontSize:"16px",fontWeight:"800",color:trendColor}},(trend>0?"+":"")+trend+"%"));
      trendEl.appendChild(el("div",{style:{fontSize:"9px",color:trendColor}},"Tendencia"));
      rr.appendChild(trendEl);
    }
    ...
    if(scores.length>1){
      ...
      scores.forEach((s,i)=>{
        ...
        circle.setAttribute("fill",BRAND);
        svg.appendChild(circle);
      });
      card.appendChild(svg);
      // Date labels
      const dateRow=el("div",{style:{display:"flex",justifyContent:"space-between",fontSize:"10px",color:"#9ca3af",marginTop:"2px"}});
      dateRow.appendChild(el("span",null,sorted[0].date||""));
      dateRow.appendChild(el("span",null,sorted[sorted.length-1].date||""));
  ...
  if (req.method === "POST" && url === "/api/auth") {
    ...
    req.on("end", () => {
      try {
        ...
        const correct = process.env.QA_PASSWORD || "vamos2026";
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ ok: password === correct }));
      } catch(e) {
