...
REFERRAL PROGRAM: $50 cash per referral (not same household). Always mention as a coaching suggestion at close — but do NOT score this as a fail if not mentioned.
...
5. Build value through benefits and solutions connected to the prospect's specific problems.
6. Discover pain FIRST before presenting anything.

OBJECTION TOOLS (NO payment plans — never penalize for not offering):
1. Value through benefits connected to prospect's problems
2. Urgency (limited spots)
3. Referral bonus ($50) — coaching suggestion only, not scored
...
GREETING (4 criteria):
1. Introduced with a name (first name sufficient) and mentioned Vamos Health. If this is a callback or inbound call where conversation resumes mid-context, score "na" — reintroduction not required.
2. Established purpose within first 20 seconds. If this is an inbound call where the PROSPECT states the reason for calling, score "na" — rep does not need to establish purpose when prospect already did.
...
2. Asked how the prospect currently manages their healthcare — do they have a regular clinic, pay cash, have insurance, or just wait until they go to the ER? This is the critical pivot that shapes the entire presentation.
...
2. Built value through benefits and solutions connected to the prospect's specific problems — presented Vamos as the direct answer to what the prospect described
...
3. Addressed concerns with value and solutions — NOT payment plans
4. Used approved tools in order
...
RAPPORT (5 criteria):
1. Empathetic tone throughout
2. Prospect felt heard and valued
3. Prospect talked MORE than rep
4. Trust through honesty — no false promises
5. Consistent energy even through objections

MISSED OPPORTUNITIES (3 scored criteria — "fail"=missed, "pass"=handled/not applicable):
...
COACHING NOTE (not scored): Always mention in coaching if $50 referral bonus was not mentioned at close — frame it as a positive opportunity, never as a failure.

FEEDBACK: Every "fail" note must: (1) name exact transcript moment, (2) give word-for-word script, (3) include real sales stat with source.

STATS: 4+ discovery questions=43% more closes (RAIN 2023). Pain verbalized=2.4x more likely (Gong 2022). Specific next step=3x better conversion (HubSpot 2023). Rep <40% talk=54% more closes (Gong 2021). Referral mention=45% more referrals (Wharton). Same-day close=67% more (InsideSales). Personalized=36% higher (Salesforce 2023). Questions for objections=70% retention (Richardson).

COACHING SUMMARY (Spanish and English):
1. Lo que hiciste bien — 1-2 specific wins
2. Tus 3 áreas — exact moment + word-for-word script + stat
3. Lo que esto impacta — one sentence on closes/retention/reactivations
If $50 referral bonus was not mentioned, add it as a coaching tip here.
...

{
  ...
  "categories": {
    ...
    "opportunities": { "scores": ["na","na","na"],                "notes_es": ["","",""],           "notes_en": ["","",""] }
  }
}`;
...
const CATS=[
  ...
  {id:"objections",label:"Objeciones",label_en:"Objections",color:"#D85A30",bg:"#FAECE7",tc:"#712B13",
   es:["Recibió objeciones con calma — 'Entiendo', 'Tiene sentido'","Clarificó la objeción antes de responder","Respondió con valor y soluciones — NO planes de pago","Usó herramientas: valor → urgencia → descuento familiar","Confirmó resolución antes de continuar"],
   en:["Received objections calmly — 'Entiendo', 'Tiene sentido'","Clarified objection before responding","Addressed with value and solutions — NOT payment plans","Used tools: value → urgency → family discount","Confirmed resolution before moving on"]},
  ...
  {id:"opportunities",label:"Oportunidades Perdidas",label_en:"Missed Opportunities",color:"#888780",bg:"#F1EFE8",tc:"#2C2C2A",
   es:["Señal de compra ignorada — interés no capitalizado","Oportunidad familiar perdida — no se exploró plan o descuentos","Dolor mencionado de pasada sin profundizar"],
   en:["Buying signal ignored — prospect interest not capitalized","Family opportunity missed — plan or discounts not explored","Pain mentioned but not explored"]}
];
...
function renderDashboard(app){
  ...
  [{label:"Hoy",value:d.callsToday,color:"#EC4899"},{label:"Esta semana",value:d.callsThisWeek,color:"#9D174D"},{label:"Este mes",value:d.callsThisMonth,color:"#7F77DD"},{label:"Ventas cerradas",value:weekCloseRate+"% sem / "+monthCloseRate+"% mes",color:"#085041"}].forEach(s=>{
  ...
  function teamCol(title,teamStats,repStats){
    const col=el("div");
    col.appendChild(el("div",{style:{fontWeight:"700",fontSize:"12px",color:"#9D174D",background:"#FDF2F8",borderRadius:"6px",padding:"4px 10px",marginBottom:"8px",textAlign:"center"}},title));
    if(teamStats&&teamStats.length){
      const tw=el("div",{style:{background:"#fff",border:"1px solid #e5e7eb",borderRadius:"10px",padding:"10px",marginBottom:"10px"}});
      teamStats.forEach((t,i)=>{
        ...
        const bw=el("div",{style:{flex:"1",height:"6px",background:"#f3f4f6",borderRadius:"3px",overflow:"hidden"}});
        bw.appendChild(el("div",{style:{height:"100%",width:avg+"%",background:avg>=80?"#5DCAA5":avg>=60?"#EC4899":"#EF4444",borderRadius:"3px"}}));
        row.appendChild(bw);
    ...
    if(repStats&&repStats.length){
      const rw=el("div",{style:{background:"#fff",border:"1px solid #e5e7eb",borderRadius:"10px",padding:"10px"}});
  ...
  const teamMap={};
  allTeams.forEach(t=>{if(!teamMap[t.team]||parseFloat(t.avg_score)<parseFloat(teamMap[t.team].avg_score))teamMap[t.team]=t;});
  ...
  if(needCoaching.length){
    ...
    const ncWrap=el("div",{style:{background:"#FEE2E2",border:"1px solid #EF4444",borderRadius:"10px",padding:"12px"}});
...
function renderHistory(app){
  ...
  state.hist.forEach(h=>{
    ...
    if(h.callUrl)meta.appendChild(el("a",{href:h.callUrl,target:"_blank",style:{marginLeft:"6px",fontSize:"10px",color:"#EC4899"}},"🔗"));
